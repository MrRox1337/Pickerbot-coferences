...
MSc Robotics, Middlesex University Dubai, Dubai, UAE}

m00983641@mdx.ac.ae, m01088120@mdx.ac.ae
}
...
To establish this mapping, a 77-point calibration grid is used, arranged as 11 columns and 7 rows on the workspace surface with 20\,mm spacing (Fig.~\ref{fig:calibration_grid}). The world $X$-coordinate of each column decreases from $+100$\,mm to $-100$\,mm left to right, and the world $Y$-coordinate increases from 410\,mm to 530\,mm top to bottom, yielding a calibrated area of $200\,\text{mm} \times 120\,\text{mm}$. The operator captures a reference image of the grid and uses \texttt{tools/calibration\_clicker.py} to click each of the 77 intersection points. A second utility, \texttt{tools/sort\_and\_tag\_pixels.py}, sorts the recorded pixel coordinates and appends the corresponding world coordinates to produce a four-column CSV file.
